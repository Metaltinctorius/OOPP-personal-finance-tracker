...
public class TransactionHandler {
  ...
  public void insertTransaction(List<String> args) {
    Transaction transaction =
        parseTransactionArguments(args); // Parses the input, returns a Transaction object
    Document doc =
        DocumentTransformer.toDocument(
            transaction); // Creates the document that can be stored in the database
  ...
  public Transaction parseTransactionArguments(List<String> args) {
    ...
    double amount =
        validateAmount(
            Double.parseDouble(
                args.get(
                    0))); // parses the string to a double, validates the double, then stores it.
    TransactionType type =
        parseAndValidateType(args.get(1)); // TODO: Implement a string parser for the type.
    LocalDate date =
        parseAndValidateDate(
            (args.get(
                2))); // Parses the string and validates it, creating an object of type LocalDate.
